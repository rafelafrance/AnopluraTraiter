...
def main(args):
    ...
    we need.
    """
    with t_util.get_temp_dir(
            where=args.temp_dir,
            keep=args.keep_temp_dir
            ) as temp_dir:
        page_file = NamedTemporaryFile(
            suffix='.html',
            mode='r',
            dir=temp_dir,
            delete=(not args.keep_temp_dir))
